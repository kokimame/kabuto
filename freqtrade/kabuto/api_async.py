...
class AsyncAPI(API):
    ...
    def __init__(self, config={}):
        ...
        super(AsyncAPI, self).__init__(config)
        self.throttle = None
    ...
    async def fetch2(self, path, api='public', method='GET', params={}, headers=None, body=None,
        ...
        return await self.fetch(request['url'], request['method'], request['headers'],
                                request['body'])
    ...
    async def fetch(self, url, method='GET', headers=None, body=None):
        ...
        try:
            async with session_method(yarl.URL(url, encoded=True),
                ...
                http_response = self.on_rest_response(http_status_code, http_status_text, url,
                                                      method, headers, http_response,
                ...
                self.logger.debug("%s %s, Response: %s %s %s", method, url, http_status_code,
        ...
        self.handle_errors(http_status_code, http_status_text, url, method, headers, http_response,
                           json_response, request_headers, request_body)
    ...
    async def fetch_deposit_address(self, code, params={}):
        if self.has['fetchDepositAddresses']:
            ...
            if deposit_address is None:
                raise NotSupported(
                    self.id + ' fetch_deposit_address could not find a deposit address for ' + code + ', make sure you have created a corresponding deposit address in your wallet on the exchange website')
            else:
